...
Game::Game(const string& path_to_map, int visual) : path_to_map(path_to_map), visual(visual) {}
...
int Game::getStage(int w) {
    ...
    for (size_t i {0}; i < stage_indices.size(); i++) {
        if (w >= stage_indices.at(i) &&
            (i + 1 >= stage_indices.size() || w < stage_indices.at(i + 1))) {
            return i; // Return the stage number
        }
    }
    throw std::runtime_error("Invalid stage index: " +
                             std::to_string(w)); // Error if no valid stage found
}
...
void Game::createMap(const vector<string>& map_lines) {
    ...
    for (size_t h {1}; h < map_lines.size(); h++) {
        for (size_t w {0}; w < map_lines.at(h).size(); w++) {
            ...
            if (map_lines.at(h).at(w) == 'v' || map_lines.at(h).at(w) == '>' ||
                map_lines.at(h).at(w) == '<' || map_lines.at(h).at(w) == '^') {
                // found a player character
                player =
                    Player(h - 1, w,
                           map_lines.at(h).at(w)); // Ensure Player class has a matching constructor
            } else if (map_lines.at(h).at(w) == '0') // Check for end position
            {
                ...
                if (food_count.find(temp_stage) !=
                    food_count.end()) // Check if the key exists in the map
                {
                    food_count[temp_stage]++; // Increment the key value
                } else {
                    food_count[temp_stage] = 1; // Initialize the key value
                }
            } else if (map_lines.at(h).at(w) == 'A' ||
                       map_lines.at(h).at(w) == 'B') // Check for food entity
            {
...
void Game::loadMap(const string& path) {
    ...
    if (s_counter > 10) {
        throw std::runtime_error("Too many stages in map file: " + path +
                                 " Modify getStageIndices() \
...
void Game::advanceGameCycle(int action) {
    ...
    if (action == 0) {
        ...
        throw std::runtime_error("Invalid player action: " +
                                 std::to_string(action)); // Error if invalid action
    }
...
void Game::displayGame() {
    ...
    cout << "Stage: " << getStage(player.getW()) << " Score: " << score << " Moves: " << cycle
         << endl;
    string stage_text = "";
    stage_text.resize(map[0].size(), ' '); // Initialize stage text with spaces
    for (size_t i {0}; i < stage_indices.size(); i++) {
        stage_text[stage_indices[i]] =
            std::to_string(i + 1)[0]; // Fill the stage text with stage numbers
    }
    cout << stage_text << endl; // Display the stage text
    for (size_t h {0}; h < map.size(); h++) {
        for (size_t w {0}; w < map.at(h).size(); w++) {
            if (visual == 3 || visual == 4 ||
                isInVision(h, w)) // Check if the position is in vision
            {
...
void Game::movePlayer(int direction) {
    ...
    if (new_h < 0 || new_h >= static_cast<int>(map.size()) || new_w < 0 ||
        new_w >= static_cast<int>(map.at(0).size())) {
        throw std::runtime_error(
            "Invalid player movement: out of bounds"); // Error if out of bounds
    }
