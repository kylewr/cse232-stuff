...
Enemy::Enemy(int h, int w, string enemy_type) : type(enemy_type), pos {h, w} {
    if (type == "vertical") {
    ...
    if (type == "vertical") {
        ...
        if (direction == 'v') {
            ...
            throw std::runtime_error(
                "Invalid direction for vertical enemy"); // Error if direction is invalid
        }
        ...
        } else if (player.getW() == new_w &&
                   player.getH() == new_h) // Check if the enemy hits the player
        {
